...
const addOne = (arr) => {
  var newArr = [];
  arr.forEach(index => newArr.push(index + 1));
  return newArr;
};
...
const allUpperCase = (arr) => {
  let newArr = [];
  arr.forEach(r => newArr.push(r.toUpperCase()));
  return newArr;
};
...
const speaker = (words, callback) => {
  let newArr = [];
  words.forEach(r => newArr.push(callback(r)));
  return newArr;
};
...
Write a function named fizz buzz that takes in an array of numbers.
