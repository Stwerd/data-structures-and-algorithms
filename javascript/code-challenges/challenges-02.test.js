...
const raisedToTheThird = (arr) => {
  let newArr = arr.map(r => Math.pow(r, 3));
  return newArr;
};
...
const mapTwoToThe = (arr) => {
  let newArr = arr.map(r => Math.pow(2,r));
  return newArr;
};
...
const charCode = (arr) => {
  let newArr = arr.map(r=> r.charCodeAt());
  return newArr;
};
...
const extractAbilities = (arr) => {
  let newArr = arr.map(r =>r.ability.name);
  return newArr;
};
