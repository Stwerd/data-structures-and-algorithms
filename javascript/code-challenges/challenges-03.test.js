...
const typeNum = (arr) => {
  return arr.filter(r => typeof r === 'number');
};
...
const containsAnd = (arr) => {
  return arr.filter(r => typeof r === 'string' && r.includes('and'));
};
...
const evenOddNumericValues = (arr) => {
  let newarr = arr.filter(r=>typeof(r) === 'number');
  let oddevenarr = newarr.map(r=> {
    if(r % 2===0)
      return 'even';
    else
      return 'odd';
