...
const screenForNames = (arr) => {
  let regex = /^((M(r(|s)|s))|(Dr)). [a-zA-Z]+/gm;
...
let biggerThanLuke = (arr) => {
  let pushed = arr.filter(r => r.mass > 77);
  return pushed.map(r => {
    return r.name;
  }).join(' - ');
};
...
const sortBy = (property, arr) => {
  // Solution code here...
  let regex = /[0-9]/g;
  return regex.test(arr[0][property]) ? arr.sort((a, b) => a[property]-b[property]) : arr.sort((a,b)=> a[property].charCodeAt(0)-b[property].charCodeAt(0));
...
const detectTicTacToeWin = (board) => {
  ...
  for(let row in board){
    winner = checkBoard(row);
    if(winner)break;
  }
  if (!winner) {
    for (let i in board) {
      winner = checkBoard([board[0][i], board[1][i], board[2][i]]);
      if (winner) break;
    }
  }
  if (!winner) winner = checkBoard([board[0][0], board[1][1], board[2][2]]);
  if (!winner) winner = checkBoard([board[0][2], board[1][1], board[2][0]]);
  return winner;
};
...
describe('Testing challenge 1', () => {
  test('It should return a list of names', () => {
    ...
    expect(screenForNames(names)).toStrictEqual(['Mr. Brown', 'Dr. Blue', 'Ms. Black', 'Mrs. Orange', 'Mr. Pink']);
  });
});
